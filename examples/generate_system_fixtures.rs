...
fn generate(root: &Path) -> io::Result<()> {
    ...
    let qbit_files = vec![FixtureFile::new(
...
impl<'a> TorrentFixture<'a> {
    ...
    fn media_root(&self) -> &'static str {
        if self.slug.starts_with("qbittorrent") {
            "media/qbittorrent"
        } else {
            "media/rtorrent"
        }
...
fn readme(entries: &[ManifestEntry]) -> String {
    let mut out = String::from(
        "# Real Client Torrent Fixtures\n\n\
         ...
         passkeys, cookies, or credentials. Source and candidate torrents for each client keep the \
         same visible file tree and file sizes while changing only the private `source` marker in \
         the info dictionary, which gives them distinct info hashes.\n\n\
