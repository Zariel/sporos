...
pub use title::{ParsedTitle, get_media_type, parse_title, parsed_name_and_media};
use title::{
    capture_u32, clean_title, episode_match, is_video_file, normalized_query_key, season_number,
};
    ...
    F: FnMut(&Path) -> crate::Result<()>,
{
    ...
    for entry in entries {
        ...
        if file_type.is_file() && path_has_extension(&entry.path(), title::VIDEO_EXTENSIONS) {
            has_video = true;
        }
    }
    child_dirs.sort();
...
fn data_dir_single_episode_in_season_pack(searchee: &Searchee<'_>) -> bool {
    searchee.source() == crate::domain::SearcheeSource::DataDir
        ...
        && searchee
            .path
            .as_ref()
            .is_some_and(|path| title::is_season_match(path.as_ref()))
}
...
fn non_standard_naming(searchee: &Searchee<'_>) -> bool {
    matches!(searchee.media_type, MediaType::Episode | MediaType::Pack)
        && !title::is_episode_match(searchee.title.as_ref())
        && !title::is_season_match(searchee.title.as_ref())
}
...
fn season_key_from_title(title: &str) -> Option<SeasonKey> {
    let captures = title::season_captures(title)?;
