...
pub fn full_runtime(
    ...
    tracing::info!(
        ...
        "single-writer service runtime initialized"
    );
...
mod tests {
    ...
    #[test]
    fn full_runtime_allows_deployment_to_own_single_writer_contract() {
        let root = temp_path("runtime-single-writer");
        let config = test_config(root.clone());
        let hooks = CountingHooks::default();

        let runtime = full_runtime(root.clone(), config.clone(), &hooks).expect("runtime");
        let second = full_runtime(root.clone(), config.clone(), &hooks).expect("second runtime");
