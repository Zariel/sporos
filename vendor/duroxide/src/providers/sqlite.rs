...
/// Durability mode used for file-backed SQLite databases.
#[derive(Debug, Clone, Copy, Default)]
pub enum SqliteSynchronous {
    /// Sync the WAL at checkpoints but not after each transaction.
    #[default]
    Normal,
    /// Sync the WAL after each transaction.
    Full,
}
...
impl SqliteProvider {
    ...
    async fn enqueue_orchestrator_work_with_delay(
        ...
        {
            // Reserve the identity in the same transaction as the queue write. The
            // primary key makes repeated starts deterministic even before dispatch.
            sqlx::query(
                r#"
                INSERT INTO instances
                    (instance_id, orchestration_name, orchestration_version,
                     current_execution_id, parent_instance_id)
                VALUES (?, ?, ?, ?, ?)
                "#,
            )
            .bind(instance)
            .bind(orchestration)
            .bind(version.as_deref().unwrap_or("unknown"))
                ...
                Box::pin({
                    ...
                    async move {
                        ...
                        if is_memory {
                            ...
                            sqlx::query(synchronous_pragma).execute(&mut *conn).await?;
